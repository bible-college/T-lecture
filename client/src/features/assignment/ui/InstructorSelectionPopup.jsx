// src/features/assignment/ui/InstructorSelectionPopup.jsx
import React, { useState } from 'react';
import { Button } from '../../../shared/ui/Button';
import { InputField } from '../../../shared/ui/InputField';

export const InstructorSelectionPopup = ({ target, onClose, onAdd }) => {
    ...
    const allInstructors = [
        ...
        { id: 3, name: '박민수', team: '교육3팀' } // 이 날짜에 불가능한 강사 포함
    ];
                ...
                <div className="p-4 flex-1 flex flex-col h-[300px]">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-1">
                        {filteredList.map(inst => (
                            <div key={inst.id} className="flex justify-between items-center p-2 hover:bg-indigo-50 rounded cursor-pointer group border border-transparent hover:border-indigo-100">
                                <div>
                                    <div className="text-sm font-bold text-gray-800">{inst.name}</div>
                                    <div className="text-xs text-gray-500">{inst.team}</div>
                                </div>
                                <Button size="xsmall" variant="outline" onClick={() => {
                                    alert(`${inst.name} 강사를 추가합니다.`);
                                    onClose();
                                }}>선택</Button>
                            </div>
                        ))}
                        {filteredList.length === 0 && <div className="text-center text-gray-400 text-xs mt-10">검색 결과가 없습니다.</div>}
                    </div>
                </div>
            </div>
        </div>
