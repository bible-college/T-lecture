...
/**
 * 읽기 전용 미니 캘린더
 * @param {string[]} availableDates - ['2025-05-01', '2025-05-02'] 형태의 날짜 배열
 * @param {string} className - 추가 스타일 클래스
 * @param {string} width - 캘린더 너비 (예: '240px', '100%', '18rem')
 */
export const MiniCalendar = ({ availableDates = [], className = '', width = '240px' }) => {
    return (
        // 1. wrapper에 style={{ width }}를 적용하여 외부에서 크기 제어
        <div 
            className={`mini-calendar-wrapper bg-white rounded-lg shadow-xl border border-gray-200 p-2 ${className}`}
            style={{ width: width }}
        >
            <style>{`
                /* 2. 캘린더 본체는 부모(wrapper) 너비를 가득 채우도록 설정 */
